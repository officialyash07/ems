...
import { TrendingUp, Users, BarChart3, Clock, AlertCircle, Loader } from "lucide-react";
...
const ManagerAnalytics = () => {
    ...
    const fetchAnalyticsData = async () => {
        try {
            ...
            const uniqueEmployees = new Set(allTasks.map(t => t.assignedToId).filter(Boolean)).size;
            ...
            allTasks.forEach(task => {
                if (task.assignedToId) {
                    memberTasks[task.assignedToId] = (memberTasks[task.assignedToId] || 0) + 1;
                }
            });
            
            const memberPerformanceData = Object.entries(memberTasks)
                .map(([name, tasks]) => ({ name: name.replace('intern-', 'Int. '), tasks }))
            ...
            setMetrics({
                ...
                taskTrendData,
                taskStatusData,
                memberPerformanceData: memberPerformanceData.length > 0 ? memberPerformanceData : 
