...
const members = [
  {
    id: "announcements",
    name: "Announcements",
    avatar: "https://ui-avatars.com/api/?name=An&background=0D8ABC&color=fff",
  },
  {
    id: "tech_support",
    name: "Tech Support",
    avatar: "https://ui-avatars.com/api/?name=TS&background=f59e0b&color=fff",
  },
];

const initialMessages = {
  announcements: [
    { from: "me", text: "Welcome to the Team Announcements channel." },
  ],
  tech_support: [
    { from: "them", text: "Welcome to Tech Support. How can we help you?" },
  ],
};
    ...
    <div className="flex h-[calc(100vh-6rem)] rounded-2xl border border-slate-200 bg-white overflow-hidden">
          ...
          {members.map((member) => (
            <button
              ...
              className={`flex w-full items-center gap-3 p-4 text-left hover:bg-slate-100 transition ${
                activeMember.id === member.id ? "bg-slate-100" : ""
              }`}
            >
              <Avatar user={member} />
      ...
      <div className="flex flex-1 flex-col">
        ...
        <div className="flex items-center gap-3 border-b border-slate-200 p-4">
          ...
          <div>
            <p className="font-medium text-slate-900">{activeMember.name}</p>
            <p className="text-sm text-slate-500">{activeMember.role}</p>
          </div>
        </div>
