...
const TlSubmissionsReview = () => {
            ...
            for (const task of tasks) {
                try {
                    ...
                    const mapped = taskSubmissions.map(sub => ({
                        ...
                        type: sub.fileUrl ? 'File Upload' : sub.externalLink ? 'External Link' : 'Comment',
                        submittedAt: new Date(sub.createdAt).toLocaleDateString(),
                        statusBadge: sub.status === 'pending' ? 'Pending Review' : 
                                    sub.status === 'approved' ? 'Approved' : 'Changes Requested',
                    }));
    ...
    const updateStatus = async (status) => {
        ...
        try {
            console.log('[updateStatus] Starting status update:', { submissionId: active.id, newStatus: status });
            ...
            // Update local state
            setSubmissions((prev) =>
                prev.map((s) => (s.id === active.id ? { ...s, statusBadge: status } : s)),
            );
